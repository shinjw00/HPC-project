...
class Actor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.conv1 = nn.Conv2d(state_dim[2], 8, kernel_size = 3, stride = 1, padding = 1)
        self.conv2 = nn.Conv2d(8, 16, kernel_size = 3, stride = 1, padding = 1)
        self.conv3 = nn.Conv2d(16, 32, kernel_size = 3, stride = 1, padding = 1)
        
        self.linear_input_size = 32 * state_dim[0] * state_dim[1]
        
        self.layer_1 = nn.Linear(self.linear_input_size, 256)
    ...
    def forward(self, x):
        ...
        if x.shape[0] != batch_size:
            ...
            x = x
        x = x.permute(0, 3, 1, 2)  # reshape from (batch, height, width, channels) to (batch, channels, height, width)
    ...
    def sample_log_prob(self, state):
        ...
        return action, log_prob.sum(dim=1, keepdim=True), std.mean()
...
class Critic(nn.Module):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.layer_1 = nn.Linear(state_dim[0] * state_dim[1] * state_dim[2] + action_dim, 256)
    self.layer_2 = nn.Linear(256, 256)
    self.layer_3 = nn.Linear(256, 1)     # Q는 한개만 ouput으로 나오므로 Q_dim일 필요가 없다. Q_dim = 1 
    # Defining the second Critic neural network
    self.layer_4 = nn.Linear(state_dim[0] * state_dim[1] * state_dim[2] + action_dim, 256)
...
class SAC(object):
    ...
    def train(self, iterations, replay_buffer, batch_size, alpha_1, discount_factor, policy_freq, tau):
        for it in range(iterations):
            ...
            # Critic training
            next_action, next_log_prob, _ = self.actor.sample_log_prob(next_state)
            ...
            # Actor training
            if it % policy_freq == 0:
                _, log_prob, _ = self.actor.sample_log_prob(state)
        ...
        _, _ , std = self.actor.sample_log_prob(state)
                    
        return log_prob.mean(), critic_loss, actor_loss, std.mean()

env = gym.make("CarRacing-v2", render_mode = 'human')

state_dim = env.observation_space.shape    # (96, 96, 4)
...
for t in range(total_timesteps):
    ...
    if t < start_timesteps:
        ...
        action = policy.select_action(obs)    # print(obs.shape) (96, 96, 3)
    ...
    if t >= start_timesteps and t % update_freq ==0:
        ...
        log_prob, critic_loss, actor_loss, std = policy.train(iterations, replay_buffer, batch_size, alpha_1, discount_factor, policy_freq, tau)
        timestep_1.append(t)
        log_prob_timestep.append(log_prob.detach().numpy())
        critic_loss_timestep.append(critic_loss.detach().numpy())
        actor_loss_timestep.append(actor_loss.detach().numpy())
        std_timestep.append(std.detach().numpy())
        
    env.render()
  
# plt.plot(timestep, reward_timestep, 'r--')
# plt.plot(timestep_1, log_prob_timestep, 'g--')
# plt.plot(timestep_1, critic_loss_timestep, 'b--')
# plt.plot(timestep_1, actor_loss_timestep, 'y--')
plt.plot(timestep_1, std_timestep, 'k--')
plt.xlabel('Timestep')
plt.ylabel('Variable')
plt.title('Variable over Timesteps')
plt.show()  
